...
"${CONTAINER_ENGINE}" run --rm \
  ...
  -e PYTORCH_BUILD_VERSION="${PYTORCH_BUILD_VERSION:-}" \
  -e PYTORCH_BUILD_NUMBER="${PYTORCH_BUILD_NUMBER:-}" \
  -e AUDITWHEEL_PLAT="${MANYLINUX_PLAT}" \
  -e USE_AVX="${USE_AVX:-0}" \
  -e CLEAN_BUILD="${CLEAN_BUILD:-1}" \
  -e PYTHON_ONLY_REPACK="${PYTHON_ONLY_REPACK:-0}" \
  -e USE_MSLK="${USE_MSLK:-0}" \

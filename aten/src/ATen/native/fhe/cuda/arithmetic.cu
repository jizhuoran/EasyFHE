...
__global__ void add_scalar_mod_kernel(
    uint64_t* c,
    const uint64_t* a,
    const uint64_t scalar,
    uint64_t mod) {
  auto i = blockIdx.x * blockDim.x + threadIdx.x;
  c[i] = add_mod(a[i], scalar, mod);
}

__global__ void add_scalar_mod_kernel_(
    uint64_t* self,
    const uint64_t scalar,
    uint64_t mod) {
  auto i = blockIdx.x * blockDim.x + threadIdx.x;
  self[i] = add_mod(self[i], scalar, mod);
}
...
static void add_scalar_mod_template(
    Tensor& c,
    const Tensor& a,
    uint64_t scalar,
    uint64_t mod) {
  AT_DISPATCH_V2(
      ...
      AT_WRAP([&]() {
        ...
        auto grid = (N + block_work_size() - 1) / block_work_size();
        auto stream = at::cuda::getCurrentCUDAStream();
        fhe::add_scalar_mod_kernel<<<grid, block_work_size(), 0, stream>>>(
            c_ptr, a_ptr, scalar, mod);
        C10_CUDA_KERNEL_LAUNCH_CHECK();
      }),
      kUInt64);
}
...
static void add_scalar_mod_template_(
    Tensor& self,
    uint64_t scalar,
    uint64_t mod) {
  AT_DISPATCH_V2(
      self.scalar_type(),
      "add_scalar_mod_cuda_",
      AT_WRAP([&]() {
        auto self_ptr = reinterpret_cast<uint64_t*>(self.data_ptr<uint64_t>());
        auto N = self.numel();
        TORCH_INTERNAL_ASSERT(
        ...
        auto grid = (N + block_work_size() - 1) / block_work_size();
        auto stream = at::cuda::getCurrentCUDAStream();
        fhe::add_scalar_mod_kernel_<<<grid, block_work_size(), 0, stream>>>(
            self_ptr, scalar, mod);
        C10_CUDA_KERNEL_LAUNCH_CHECK();
      }),
      kUInt64);

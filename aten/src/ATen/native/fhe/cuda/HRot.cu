...
__global__ void hrot_sum_reduce_p_from_modup_kernel(
  ...
  out_ax[out_i] = barrett_reduction_128_64(accum_ax, prime, barret_ratio, barret_k);
  out_bx[out_i] = barrett_reduction_128_64(accum_bx, prime, barret_ratio, barret_k);
}
...
__global__ void hrot_ntt_phase2_finalize_kernel(
  ...
  const auto base_bx = ntt::phase2_pair<LOG_N, NUM_WARP>(
      workspace,
      limb,
      blockIdx.x,
      power_of_roots,
      ...
      tile);
  __syncthreads();
  const auto base_ax = ntt::phase2_pair<LOG_N, NUM_WARP>(
      workspace + cv_stride,
      limb,
      blockIdx.x,
      power_of_roots,

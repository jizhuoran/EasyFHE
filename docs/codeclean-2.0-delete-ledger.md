...
| Deleted | `easyfhe/_C_flatbuffer/**` | Flatbuffer typing stub package was only a JIT/mobile serialization typing surface after public JIT/export/mobile helpers had already been removed. | Stale `_C/__init__.pyi*` entries still mention JIT/mobile flatbuffer serialization and should be cleaned in the native/stub audit; native JIT serialization headers remain under installed `include/` until build payload trimming. |
| Deleted | `easyfhe/autograd/forward_ad.py`, `easyfhe/autograd/functional.py`, `easyfhe/autograd/gradcheck.py`, `easyfhe/autograd/anomaly_mode.py`, `easyfhe/autograd/variable.py`, `easyfhe/autograd/_functions/**` | Disabled backward/forward training compatibility surfaces removed. `autograd/__init__.py` keeps only profiler/native init, no-grad/inference-mode helpers, `Variable = Tensor`, and a tiny `forward_ad.unpack_dual` compatibility object used by tensor string formatting. Deprecated non-inplace `Tensor.resize/resize_as` now raise directly instead of importing disabled autograd `_functions`. | Native follow-up anchors: generated autograd Python bindings, autograd node/function type stubs, saved-tensor hook bindings used only by disabled training flows, and `torch/csrc/autograd/**` paths that are not needed by profiler or tensor versioning. Keep profiler record-function/Kineto bindings and grad-mode/native tensor guards. |
...
| Native deleted | `aten/src/ATen/native/cuda/linalg/**`, `aten/src/ATen/native/kleidiai/**` | First ATen source deletion batch after the matching Python surfaces were removed. Both were absent from the fresh fast-build dependency path. | Verified with `rm -rf build` followed by `BUILD_CUSTOM_PROTOBUF=OFF MAX_JOBS=$(nproc) tools/easyfhe_fast_build.sh`; import/NumPy/NCCL/profiler smoke and CUDA ResNet20 AESPA passed. `libtorch_cuda.so` still lists `libcusolver.so` as an unused direct dependency, so CUDA link-dependency trimming is a later build-system cleanup. |

...
- Current build compiles 251 tracked source files under `aten/src/ATen`, out of
  roughly 948 tracked C/C++/CUDA source files there. Most pruning already
  happens through the EasyFHE fast-build source filters in `caffe2/CMakeLists.txt`.
...
- `aten/src/ATen/native/cpu/*.cpp` files are not compiled in the current fast
  build, but some headers under `native/cpu/` are included by compiled dense
  tensor code such as `TensorShape.cpp`, `Distributions.cpp`, and `SharedReduceOps.h`.
...
| --- | ---: | --- | --- |
| `aten/src/ATen/native/cpu/**` | 716 KiB | no `.cpp` compiled | Headers are still included; delete source files before headers. |
...
| `aten/src/ATen/native/cuda/linalg/**` | 68 KiB | not compiled | Good source-level deletion batch. |
| `aten/src/ATen/native/kleidiai/**` | 44 KiB | not compiled | Good source-level deletion batch. |
...
| `aten/src/ATen/native/cpu/{CatKernel.h,SerialStackImpl.h,StackKernel.h,Loops.h,zmath.h,utils.h}` and similar headers | Keep for now because compiled dense/native code still includes them even though the CPU kernel `.cpp` files are pruned. |
...
| Uncompiled `aten/src/ATen/native/cpu/*.cpp` files | Current fast build excludes these generated/selected CPU kernels and ResNet uses CUDA. | Delete `.cpp` files only, keep headers first. Rebuild `torch_cpu torch_cuda`; smoke `torch.tensor`, `from_numpy`, `tensor.numpy`, profiler, and GPU ResNet. |
...
1. **Inactive external/math backends**
   - Delete `aten/src/ATen/native/cuda/linalg/**`.
   - Delete `aten/src/ATen/native/kleidiai/**`.
   - Build target: `torch_cuda torch_python`.
   - Extra check: after the build, inspect whether `libtorch_cuda.so` still links
     `libcusolver.so`; if yes, trim the CUDA dependency list separately.
2. **CPU kernel source-only pass**
   - Delete uncompiled `.cpp` files under `aten/src/ATen/native/cpu/`.
   - Keep all `native/cpu/*.h` first, because compiled dense code includes
     several of them.
   - Build target: `torch_cpu torch_cuda torch_python`.
3. **Special/math CUDA source-only pass**
...
2. CPU/source-only cleanup:
   - delete uncompiled `aten/src/ATen/native/cpu/*.cpp` files while keeping headers
3. Special/math CUDA source-only cleanup:
   - delete uncompiled CUDA special/math kernel families that match removed public `special` surface
4. FFT native cleanup:

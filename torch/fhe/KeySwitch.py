...
def Decompose_pad(axax, curr_limbs, K, N):
    beta = int(math.ceil(curr_limbs / K))  # total beta groups
    ceil_curr_limbs = int(beta) * K
    d2Tilde = np.zeros((beta, ceil_curr_limbs + K, N), dtype=np.uint64)
...
def ModUp_pad(a, d2Tilde,
              moduliQ, qInvVec, qRootScalePows, qRootScalePowsInv, NScaleInvModq, QHatInvModq,
              moduliP, pInvVec, pRootScalePows, QHatModp,
              curr_limbs, K, N):
    beta = math.ceil(curr_limbs / K)
    ceil_curr_limbs = int(beta) * K
    intt_a = np.zeros((ceil_curr_limbs, N), np.uint64)

    for i in range(ceil_curr_limbs):
        intt_a[i] = arithmetic.iNTT(a[i], N, moduliQ[i], qInvVec[i], qRootScalePowsInv[i], NScaleInvModq[i])

    arithmetic.ModUp_Core_pad(intt_a, d2Tilde,
                              moduliQ, moduliP, QHatInvModq, QHatModp,
                              curr_limbs, K, N)

    moduliQP = np.concatenate((moduliQ[:ceil_curr_limbs], moduliP))
    qpInv = np.concatenate((qInvVec[:ceil_curr_limbs], pInvVec))
    qpRSP = np.concatenate((qRootScalePows[:ceil_curr_limbs], pRootScalePows))
    for j in range(beta):
        in_C_L_index = j * K
...
def InnerProduct_pad(d2Tilde, key,
                     moduliQ, moduliP,
                     curr_limbs, K, N):
    beta = math.ceil(curr_limbs / K)
    ceil_curr_limbs = int(beta) * K
    sumMult = np.zeros((2, ceil_curr_limbs + K, N), dtype=np.uint64)
    product = np.zeros((2, ceil_curr_limbs + K, N), dtype=np.uint64)
    L = len(moduliQ)
    ...
    for k in range(2):  # deal with 2 dim of a ct
        for j in range(beta):
            for i in range(ceil_curr_limbs):
                product[k][i] = arithmetic.vec_mul_mod(d2Tilde[j][i], key[k][j][i], moduliQ[i])
                sumMult[k][i] = arithmetic.vec_add_mod(sumMult[k][i], product[k][i], moduliQ[i])
            for i, pi in zip(range(ceil_curr_limbs, ceil_curr_limbs + len(moduliP)), range(len(moduliP))):
                product[k][i] = arithmetic.vec_mul_mod(d2Tilde[j][i], key[k][j][L + pi], moduliP[pi])
                sumMult[k][i] = arithmetic.vec_add_mod(sumMult[k][i], product[k][i], moduliP[pi])
...
def ModDown_pad(a,
    ...
    beta = math.ceil(curr_limbs / K)
    ceil_curr_limbs = int(beta) * K
    intt_a = np.zeros((ceil_curr_limbs+K, N), np.uint64)

    moduliQP = np.concatenate((moduliQ[:ceil_curr_limbs], moduliP))
    qpInv = np.concatenate((qInvVec[:ceil_curr_limbs], pInvVec))
    qpRSPInv = np.concatenate((qRootScalePowsInv[:ceil_curr_limbs], pRootScalePowsInv))
    qpNScaleInvMod = np.concatenate((NScaleInvModq[:ceil_curr_limbs], NScaleInvModp))
    for i in range(ceil_curr_limbs+K):
        intt_a[i] = arithmetic.iNTT(a[i], N, moduliQP[i], qpInv[i], qpRSPInv[i], qpNScaleInvMod[i])

    res = arithmetic.ModDown_Core_pad(intt_a,
                 pHatInvModp, pHatModq, PInvModq,
                 moduliQ, moduliP,
                 N, curr_limbs, K,)
...
def ModDown_pad_ct(input,
               moduliQ, qInvVec, qRootScalePows, qRootScalePowsInv, NScaleInvModq, pHatModq, PInvModq,
               moduliP, pInvVec, pRootScalePowsInv, NScaleInvModp, pHatInvModp,
               curr_limbs, K, N):
    beta = math.ceil(curr_limbs / K)
    ceil_curr_limbs = int(beta) * K
    res = np.zeros((2, ceil_curr_limbs, N), np.uint64)
    res[0] = ModDown_pad(input[0], moduliQ, qInvVec, qRootScalePows, qRootScalePowsInv, NScaleInvModq, pHatModq,
                               PInvModq, moduliP, pInvVec, pRootScalePowsInv, NScaleInvModp, pHatInvModp, curr_limbs, K, N)

    res[1] = ModDown_pad(input[1], moduliQ, qInvVec, qRootScalePows, qRootScalePowsInv, NScaleInvModq, pHatModq,
                               PInvModq, moduliP, pInvVec, pRootScalePowsInv, NScaleInvModp, pHatInvModp, curr_limbs, K, N)
    return res

def KeySwitch_core_pad(axax, swk,
                       moduliQ, qInvVec, qRootScalePows, qRootScalePowsInv, NScaleInvModq, QHatInvModq, pHatModq,
                       PInvModq,
                       moduliP, pInvVec, pRootScalePows, pRootScalePowsInv, QHatModp, NScaleInvModp, pHatInvModp,
                       curr_limbs, K, N):
    d2Tilde = Decompose_pad(axax, curr_limbs, K, N)
    d2Tilde = ModUp_pad(axax, d2Tilde,
                        moduliQ, qInvVec, qRootScalePows, qRootScalePowsInv, NScaleInvModq, QHatInvModq,
                        moduliP, pInvVec, pRootScalePows, QHatModp,
                        curr_limbs, K, N)

    sumMult = InnerProduct_pad(d2Tilde, swk,
                               moduliQ, moduliP,
                               curr_limbs, K, N)

    res = ModDown_pad_ct(sumMult,
                     moduliQ, qInvVec, qRootScalePows, qRootScalePowsInv, NScaleInvModq, pHatModq, PInvModq,
                     moduliP, pInvVec, pRootScalePowsInv, NScaleInvModp, pHatInvModp,
                     curr_limbs, K, N)
    return res
...
def _to_4d(t):
    """Convert 2D (L,N) or 3D (B,L,N) tensor to 4D (1, B_or_1, L, N)."""
    if t.dim() == 2:
...
def _from_4d(t, orig_shape):
    """Restore 4D tensor to match original 2D/3D shape's row count and N."""
    if len(orig_shape) == 2:
...
def _to_2d(t, N):
    """If t is 1D, reshape to 2D (-1, N). Otherwise return as-is."""
    if t.dim() == 1:

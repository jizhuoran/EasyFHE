...
def rescale_ct(ct, cryptoContext):
    ...
    return Cipher([res0.cpu().numpy(), res1.cpu().numpy()], ct.cur_limbs - 1)
...
def ModReduce_ct(ct, levels, cryptoContext):
    ...
    for l in range(levels):
        ...
        curr_limbs -= 1
    return Cipher([res0.cpu().numpy(), res1.cpu().numpy()], curr_limbs)
...
def homo_mul(in0, in1, cryptoContext):
    ...
    tmp = Ciphertext(np.array(F.cv_keyswitch(res.cv[2], res.curr_limbs, cryptoContext)), curr_limbs = in0.curr_limbs)
    res.cv = res.cv[:2]

...
def _eval_mult_core(cipher, constant, cryptoContext):
    ...
    return _cipher_mul_scalar(cipher, factors, cryptoContext)
...
def _cipher_mul_scalar(in0, scalar, cryptoContext):
    ...
    scFactor = cryptoContext.GetScalingFactorReal(in0.cur_limbs)
    return Cipher(cv, in0.cur_limbs, in0.scaling_factor*scFactor, in0.noise_deg+1, in0.slots)
...
def homo_add_scalar_double(ct, cnst, cryptoContext):
    tmpr = _get_element_for_eval_add_or_sub(math.fabs(cnst), ct.cur_limbs, ct.noise_deg, cryptoContext)
    if cnst < 0:
        return _cipher_sub_scalar(ct, tmpr, cryptoContext)
    else:
        return _cipher_add_scalar(ct, tmpr, cryptoContext)
...
def homo_mul_scalar_int(in0, scalar, cryptoContext):
    res = _cipher_mul_scalar(in0, abs(scalar), cryptoContext)
    if scalar < 0:
...
def homo_mul_scalar_double(cipher, cnst, cryptoContext):
    if cryptoContext.rescaleTech != "FIXEDMANUAL" and cipher.noise_deg == 2:
        cipher = homo_rescale(cipher, BASE_NUM_LEVELS_TO_DROP, cryptoContext)
    factors = _get_element_for_eval_mult(cnst, cipher.cur_limbs, cryptoContext)
    return _cipher_mul_scalar(cipher, factors, cryptoContext)
...
def homo_rotate(cipher, index, cryptoContext):
    ...
    res = F.cv_keyswitch(cipher.cv[1], cipher.cur_limbs, swk[0], swk[1], cryptoContext)
    bxrot = F.cv_add(cipher.cv[0], res[0], cryptoContext.moduliQ_cuda, cipher.cur_limbs)

    cv0 = F.cv_automorphism_transform(bxrot, cipher.cur_limbs, auto_index, cryptoContext)
    cv1 = F.cv_automorphism_transform(res[1], cipher.cur_limbs, auto_index, cryptoContext)

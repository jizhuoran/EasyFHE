...
def test_KS_ModDown2():
    ...
    sumMult = KeySwitch.InnerProduct(d2Tilde, swk, moduliQ, moduliP, L, K, N)

    res_0 = KeySwitch.ModDown_method2(
        sumMult[0],
        moduliQ,
        qInvVec,
        qRootScalePows,
    ...
    res_1 = KeySwitch.ModDown_method2(
        sumMult[1],
        moduliQ,
        qInvVec,
        qRootScalePows,
...
def test_homo_add():
    ...
    ax = torch.tensor([[1, 2, 3, 4], [4, 5, 6, 4]], dtype=torch.uint64, device="cuda")
    bx = torch.tensor([[7, 8, 9, 4], [0, 2, 0, 4]], dtype=torch.uint64, device="cuda")
    a_cipher = Cipher(ax, bx, L)

    ax = torch.tensor([[4, 5, 6, 4], [4, 5, 6, 4]], dtype=torch.uint64, device="cuda")
    bx = torch.tensor([[4, 5, 6, 4], [4, 5, 6, 4]], dtype=torch.uint64, device="cuda")
    b_cipher = Cipher(ax, bx, L)

    cryptoContext = Context(2, 53, 52, 52, L, K)
    cryptoContext.moduliQ = torch.tensor([10, 11], dtype=torch.uint64, device="cuda")
    c_cipher = homo_ops.homo_add(a_cipher, b_cipher, L, cryptoContext)

    ax_ground_truth = torch.tensor([[5, 7, 9, 8], [8, 10, 1, 8]], dtype=torch.uint64)
    bx_ground_truth = torch.tensor([[1, 3, 5, 8], [4, 7, 6, 8]], dtype=torch.uint64)
    c_cipher_ground_truth = (ax_ground_truth, bx_ground_truth, L)

    assert c_cipher == c_cipher_ground_truth
...
def test_HMultCore():
    ...
    cryptoContext = Context(
        logN, 53, 52, 52, L, K, moduliQ, moduliP, rootsQ, rootsP, swk
    )
    ct = Ciphertext(cv, L)

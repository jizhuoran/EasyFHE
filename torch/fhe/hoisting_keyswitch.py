...
def key_switch_ext(cipher, cryptoContext):
    ...
    cv = [
        torch.cat((
            F.cv_mul_scalar(cv, cryptoContext.PModq_cuda, cryptoContext.moduliQ_cuda, cryptoContext.q_mu_cuda, cipher.cur_limbs),
...
def eval_fast_rotation(digits, cipher, index, need_moddown, cryptoContext):
    ...
    if need_moddown:
        ...
        # post add after ks
        result.cv[0] = F.cv_add(result.cv[0], cipher.cv[0], cryptoContext.moduliQ_cuda, cur_limbs)
        ...
        cipher_pmodup_cv0 = F.cv_mul_scalar(cipher.cv[0], cryptoContext.PModq_cuda, cryptoContext.moduliQ_cuda,
        ...
        sumMult.cv[0] = F.cv_add(sumMult.cv[0], cipher_pmodup_cv0, cryptoContext.moduliQ_cuda, cipher.cur_limbs, inplace=True)

...
try:
    from easyfhe._C._dynamo.guards import set_is_in_mode_without_ignore_compile_internals
...
def _detect_infra_mode(key):
    ...
    from easyfhe._ops import _get_dispatch_mode_pre_dispatch
...
def _unset_infra_mode(key):
    from easyfhe._ops import _get_dispatch_mode_pre_dispatch, unset_mode_pre_dispatch
...
def _push_mode(mode: TorchDispatchMode) -> None:
    ...
    from easyfhe._ops import _set_mode_pre_dispatch, get_cached_ops
...
def _pop_mode(k: DispatchKey | torch._C._TorchDispatchModeKey | None = None):
    if k == torch._C.DispatchKey.PreDispatch:  # type: ignore[attr-defined]
        from easyfhe._ops import _pop_mode_from_pre_dispatch

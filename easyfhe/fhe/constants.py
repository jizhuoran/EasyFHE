...
class ConstantBundle:
    ...
    def cache_info(self):
        return {
            ...
            "plain_bytes": _cache_nbytes(self._plain_cache),
    ...
    def _encoded_middle(self, name, slots=None, cryptoContext=None, scale=1.0, ring_dim=None):
        ...
        key = _middle_key(name, slots, ring_dim, scale)
        ...
        middle = _prepare_vector(vector, slots, ring_dim, scale)
        if _cache_middle(self.cache_mode):
...
def _middle_key(name, slots, ring_dim, scale):
    return (name, slots, _scale_key(scale), ring_dim)
...
def _prepare_vector(vector, slots, ring_dim, scale):
    if isinstance(vector, PreparedPlaintext):

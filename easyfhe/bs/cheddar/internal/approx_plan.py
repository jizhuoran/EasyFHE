...
class BootstrapApproxPlan:
    ...
    pre_chebyshev_offset: float = 0.0
    sparse_mod_factor: float = 28.0
...
def _build_ps_node_from_coefficients(coefficients, k, m):
    ...
    s2.resize(k2m2k + 1, refcheck=False)
    s2[-1] = 1.0
    return _build_root_ps_node(k, m, divqr_q, divcs_q, s2)


def _cosine_double_angle_coefficients(actual_k, degree, double_angle_iterations):
    coeff_total = int(degree) + 1
    nodes = np.array(
        [
            math.cos(math.pi * (j + 0.5) / coeff_total)
            for j in range(coeff_total)
        ],
        dtype=np.float64,
    )
    amplitude = math.pow(1.0 / (2.0 * math.pi), 1.0 / (1 << int(double_angle_iterations)))
    values = amplitude * np.cos(
        (2.0 * math.pi * float(actual_k) / (1 << int(double_angle_iterations))) * nodes
    )
    coefficients = np.zeros(coeff_total, dtype=np.float64)
    for i in range(coeff_total):
        coefficients[i] = (2.0 / coeff_total) * sum(
            values[j] * math.cos(math.pi * i * (j + 0.5) / coeff_total)
            for j in range(coeff_total)
        )
    return coefficients


def _compute_degrees_ps_for_degree(degree_value):
    ...
    if degree_value <= 48:
        return 7, 3
    raise ValueError(f"unsupported cosine double-angle degree: {degree_value}")
...
def get_bootstrap_approx_plan(secretKeyDist):
    approx_variant = os.environ.get("EASYFHE_BOOTSTRAP_APPROX", "").upper()
    if secretKeyDist == "SPARSE_TERNARY":
        sparse_mod_factor = 28.0
        if approx_variant == "COSINE_DA":
            actual_k = float(os.environ.get("EASYFHE_COSINE_DA_K", "24.0"))
            degree_value = int(os.environ.get("EASYFHE_COSINE_DA_DEGREE", "36"))
            double_angle_iterations = int(os.environ.get("EASYFHE_COSINE_DA_DOUBLE_ANGLE", "3"))
            k, m = _compute_degrees_ps_for_degree(degree_value)
            coefficients = _cosine_double_angle_coefficients(
                actual_k,
                degree_value,
                double_angle_iterations,
            )
            ps_root = _build_ps_node_from_coefficients(coefficients, k, m)
            return BootstrapApproxPlan(
                ...
                depth=int(math.ceil(math.log2(degree_value + 1)) + double_angle_iterations),
                message_scaling_factor=1.0,
                pre_chebyshev_offset=float(-0.25 / actual_k),
                sparse_mod_factor=float(actual_k),
            )
        k = 7
        m = 3
    ...
    return BootstrapApproxPlan(
        ...
        sparse_mod_factor=float(sparse_mod_factor),
    )

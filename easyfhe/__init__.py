...
from easyfhe._utils import (
    _functionalize_sync as _sync,
    _import_dotted_name,
    classproperty,
)
from easyfhe._utils_internal import (
    get_file_path,
...
from easyfhe.torch_version import __version__ as __version__


if TYPE_CHECKING:
    from easyfhe.types import Device, IntLikeType
...
if sys.platform == "win32":
    ...
    def _load_dll_libraries() -> None:
        ...
        from easyfhe.version import cuda as cuda_version
...
if (USE_RTLD_GLOBAL_WITH_LIBTORCH or os.getenv("TORCH_USE_RTLD_GLOBAL")) and (
    ...
    from easyfhe._C import *  # noqa: F403
...
class SymBool:
    ...
    def __sym_float__(self):
        ...
        """
        from easyfhe.fx.experimental.sym_node import wrap_node
...
def sym_sum(*args):
    ...
    from easyfhe.fx.experimental.sym_node import to_node, wrap_node
...
try:
    ...
    from easyfhe._C import _initExtension
except ImportError:
    import easyfhe._C as _C_for_compiled_check
...
def get_default_device() -> "torch.device":
    ...
    from easyfhe.overrides import _get_current_function_mode_stack
    from easyfhe.utils._device import DeviceContext
...
def _check_with(
    ...
    from easyfhe.fx.experimental.symbolic_shapes import expect_true
...
def _check_is_size(i, message=None, *, max=None):
    ...
    _check(i >= 0, message)
    from easyfhe.fx.experimental.symbolic_shapes import _advise_is_size
    ...
    if max is not None:
        ...
        from easyfhe.fx.experimental.symbolic_shapes import _advise_is_bounded
...
from easyfhe._tensor import Tensor  # usort: skip

# needs to be after torch.Tensor is defined to avoid circular dependencies
from easyfhe import storage as storage  # usort: skip
from easyfhe.storage import (
    _LegacyStorage,
    _StorageBase,
...
from easyfhe.serialization import load, save
...
def _manager_path():
    if platform.system() == "Windows":
        return b""
    path = get_file_path("easyfhe", "bin", "torch_shm_manager")
    prepare_multiprocessing_environment(get_file_path("easyfhe"))
    if not os.path.exists(path):
        raise RuntimeError("Unable to find torch_shm_manager at " + path)
    return path.encode("utf-8")


_previous_torch_module = sys.modules.get("torch")
_previous_torch_modules = set(sys.modules)
sys.modules["torch"] = sys.modules[__name__]
try:
    _C._initExtension(_manager_path())
finally:
    if _previous_torch_module is None:
        sys.modules.pop("torch", None)
    else:
        sys.modules["torch"] = _previous_torch_module
    _module_name = None
    for _module_name in list(sys.modules):
        if _module_name.startswith("torch.") and _module_name not in _previous_torch_modules:
            del sys.modules[_module_name]

del _manager_path, _module_name, _previous_torch_module, _previous_torch_modules
...
if TYPE_CHECKING:
    ...
    from easyfhe._C._VariableFunctions import *  # type: ignore[assignment, misc] # noqa: F403
...
from easyfhe._compile import _disable_dynamo  # usort: skip
...
from easyfhe import _VF as _VF, functional as functional  # usort: skip
from easyfhe.functional import *  # usort: skip # noqa: F403
...
from easyfhe.autograd import (  # usort: skip
...
from easyfhe import _size_docs, _storage_docs, _tensor_docs, _torch_docs
...
from easyfhe import _library as _library, _ops as _ops


# Import the ops and classes "namespace"
from easyfhe._ops import ops as ops  # usort: skip
from easyfhe._classes import classes as classes  # usort: skip
...
from easyfhe.multiprocessing._atfork import register_after_fork
...
from easyfhe.utils.dlpack import from_dlpack, to_dlpack
...
class _TorchCompileInductorWrapper:
    ...
    def __init__(self, mode, options, dynamic, name=None):
        from easyfhe._inductor.compiler_bisector import CompilerBisector
        ...
        if hasattr(torch, "version"):
            from easyfhe.torch_version import TorchVersion
    ...
    def apply_mode(self, mode: str | None):
        if mode and mode != "default":
            from easyfhe._inductor import list_mode_options
    ...
    def apply_options(self, options: dict[str, _Any] | None):
        ...
        from easyfhe._inductor import config
    ...
    def __call__(self, model_, inputs_, *, config_patches=None):
        from easyfhe._inductor.compile_fx import compile_fx
    ...
    def get_compiler_config(self):
        from easyfhe._inductor.compile_fx import get_patched_config_dict
    ...
    def reset(self):
        ...
        if "triton.cudagraphs" in self.config or config.triton.cudagraphs:
            if self.config.get("triton.cudagraphs", True):
                from easyfhe._inductor.cudagraph_trees import reset_cudagraph_trees
...
class _TorchCompileAOTInductorWrapper(_TorchCompileInductorWrapper):
    ...
    def __call__(self, model_, inputs_, *, config_patches=None):
        ...
        from easyfhe._guards import detect_fake_mode
        from easyfhe._inductor.virtualized import V
...
class _TorchCompileWrapper:
    def __init__(self, backend, mode, options, dynamic):
        from easyfhe._dynamo.backends.registry import lookup_backend
...
def compile(
    ...
    *code object*, not frame; if you dynamically create multiple copies of a
    ...
    if backend is None:
        from easyfhe._dynamo.backends.registry import get_default_backend
    ...
    if torch.compiler.is_exporting():
        from easyfhe._higher_order_ops.utils import _in_hop_compile
...
if "TORCH_CUDA_SANITIZER" in os.environ:
    import easyfhe.cuda._sanitizer as csan

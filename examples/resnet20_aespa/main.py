...
try:
    ...
    from weight_pack import WeightPack
from examples.utils.utils import read_image

# for debug
from examples.resnet.gen_aespa_weights.HerPN import get_resnet20_HerPN, change_all_HerPN_by_PAF_MutalChannel

SCRIPT_DIR = Path(__file__).resolve().parent
RESNET_DIR = SCRIPT_DIR.parent / "resnet"
DATA_DIR = os.environ.get("DATA_DIR", str(RESNET_DIR / "data"))
...
weights_path = os.environ.get(
    ...
    str(RESNET_DIR / "resnet20_aespa_weights.npz"),
)
...
def _load_reference_model():
    ...
    model_path = RESNET_DIR / "gen_aespa_weights" / "ResNet20_Aespa.pth"
...
def initial_layer(input, cryptoContext, weights):
    ...
    res = aespa_nonlinear(res, "conv1bn1", cryptoContext, weights)
    return res
...
def layer1(input, cryptoContext, weights):
    ...
    res1 = aespa_nonlinear(res1, f"layer{1}-conv{1}bn{1}", cryptoContext, weights)
    ...
    res1 = aespa_add_shortcut(res1, input, f"layer{1}-conv{2}bn{2}", cryptoContext, weights, scale)
    res1 = fhe.align_to(res1, fhe.CipherState(res1.cur_limbs - (1), res1.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    _trace_internal_feature("layer1.block1.pre_herpn2", res1, cryptoContext)
    res1 = aespa_nonlinear(res1, f"layer{1}-conv{2}bn{2}", cryptoContext, weights)
    ...
    res2 = aespa_nonlinear(res2, f"layer{2}-conv{1}bn{1}", cryptoContext, weights)
    ...
    res2 = aespa_add_shortcut(res2, res1, f"layer{2}-conv{2}bn{2}", cryptoContext, weights, scale)
    res2 = fhe.align_to(res2, fhe.CipherState(res2.cur_limbs - (1), res2.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    ...
    res2 = aespa_nonlinear(res2, f"layer{2}-conv{2}bn{2}", cryptoContext, weights)
    ...
    res3 = aespa_nonlinear(res3, f"layer{3}-conv{1}bn{1}", cryptoContext, weights)

    scale = 1  # normalized_deltas[1][5]
    res3 = conv(res3, 32, 1, 16, -1024, 3, 2, 0, scale, cryptoContext, weights)
    res3 = aespa_add_shortcut(res3, res2, f"layer{3}-conv{2}bn{2}", cryptoContext, weights, scale)
    res3 = fhe.align_to(res3, fhe.CipherState(res3.cur_limbs - (1), res3.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    res3 = homo_bootstrap(res3, cryptoContext.L, logBsSlots_list[0], levelBudget_list[0], cryptoContext)
    res3 = aespa_nonlinear(res3, f"layer{3}-conv{2}bn{2}", cryptoContext, weights)
...
def layer2(input, cryptoContext, weights):
    ...
    fullpackSx = aespa_nonlinear(fullpackSx, f"layer{4}-conv{1}bn{1}", cryptoContext, weights)
    ...
    res1 = aespa_nonlinear(res1, f"layer{4}-conv{2}bn{2}", cryptoContext, weights)

    # layer[2]block[1]
    scale = 1  # normalized_deltas[2][2]
    res2 = conv(res1, 16, 1, 32, -256, 5, 1, 0, scale, cryptoContext, weights)
    res2 = fhe.align_to(res2, fhe.CipherState(res2.cur_limbs - (1), res2.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    res2 = aespa_nonlinear(res2, f"layer{5}-conv{1}bn{1}", cryptoContext, weights)

    scale = 1  # normalized_deltas[2][3]
    res2 = conv(res2, 16, 1, 32, -256, 5, 2, 0, scale, cryptoContext, weights)
    res2 = aespa_add_shortcut(res2, res1, f"layer{5}-conv{2}bn{2}", cryptoContext, weights, scale)
    res2 = fhe.align_to(res2, fhe.CipherState(res2.cur_limbs - (1), res2.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    res2 = aespa_nonlinear(res2, f"layer{5}-conv{2}bn{2}", cryptoContext, weights)

    # layer[2]block[2]
    scale = 1  # normalized_deltas[2][4]
    res3 = conv(res2, 16, 1, 32, -256, 6, 1, 0, scale, cryptoContext, weights)
    res3 = fhe.align_to(res3, fhe.CipherState(res3.cur_limbs - (1), res3.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    res3 = aespa_nonlinear(res3, f"layer{6}-conv{1}bn{1}", cryptoContext, weights)

    scale = 1  # normalized_deltas[2][5]
    res3 = conv(res3, 16, 1, 32, -256, 6, 2, 0, scale, cryptoContext, weights)
    res3 = aespa_add_shortcut(res3, res2, f"layer{6}-conv{2}bn{2}", cryptoContext, weights, scale)
    res3 = fhe.align_to(res3, fhe.CipherState(res3.cur_limbs - (1), res3.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    res3 = homo_bootstrap(res3, cryptoContext.L-1, logBsSlots_list[0], levelBudget_list[0], cryptoContext)
    res3 = aespa_nonlinear(res3, f"layer{6}-conv{2}bn{2}", cryptoContext, weights)
...
def layer3(input, cryptoContext, weights):
    ...
    fullpackSx = aespa_nonlinear(fullpackSx, f"layer{7}-conv{1}bn{1}", cryptoContext, weights)
    ...
    res1 = aespa_nonlinear(res1, f"layer{7}-conv{2}bn{2}", cryptoContext, weights)

    scale = 1  # normalized_deltas[3][2]
    res2 = conv(res1, 8, 1, 64, -64, 8, 1, 0, scale, cryptoContext, weights)
    res2 = fhe.align_to(res2, fhe.CipherState(res2.cur_limbs - (1), res2.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    res2 = aespa_nonlinear(res2, f"layer{8}-conv{1}bn{1}", cryptoContext, weights)

    scale = 1  # normalized_deltas[3][3]
    res2 = conv(res2, 8, 1, 64, -64, 8, 2, 0, scale, cryptoContext, weights)
    res2 = aespa_add_shortcut(res2, res1, f"layer{8}-conv{2}bn{2}", cryptoContext, weights, scale)
    res2 = fhe.align_to(res2, fhe.CipherState(res2.cur_limbs - (1), res2.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    res2 = aespa_nonlinear(res2, f"layer{8}-conv{2}bn{2}", cryptoContext, weights)

    scale = 1  # normalized_deltas[3][4]
    res3 = conv(res2, 8, 1, 64, -64, 9, 1, 0, scale, cryptoContext, weights)
    res3 = fhe.align_to(res3, fhe.CipherState(res3.cur_limbs - (1), res3.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    res3 = aespa_nonlinear(res3, f"layer{9}-conv{1}bn{1}", cryptoContext, weights)

    scale = 1  # normalized_deltas[3][5]
    res3 = conv(res3, 8, 1, 64, -64, 9, 2, 0, scale, cryptoContext, weights)
    res3 = aespa_add_shortcut(res3, res2, f"layer{9}-conv{2}bn{2}", cryptoContext, weights, scale)
    res3 = fhe.align_to(res3, fhe.CipherState(res3.cur_limbs - (1), res3.noise_deg - (1)), cryptoContext)  # RESCALE ADD BY ZRJI
    res3 = aespa_nonlinear(res3, f"layer{9}-conv{2}bn{2}", cryptoContext, weights)

...
def check_encoded_equal(ptx1, ptx2, name):
    if np.equal(ptx1.cv[0].cpu().numpy().reshape(-1), ptx2.cv[0].cpu().numpy().reshape(-1)).all():
        # return
        print("Equal")
    else:
        print("Not Equal")
        print(name)
        print(ptx1.cv[0].cpu().numpy().reshape(-1)[:10])
        print(ptx2.cv[0].cpu().numpy().reshape(-1)[:10])

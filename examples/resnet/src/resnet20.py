...
def initial_layer(input, cryptoContext):
    ...
    bias = read_values_from_file("conv1bn1-bias", cryptoContext.L - res.cur_limbs, 1, res.slots, cryptoContext, scale)
...
def layer1(input, cryptoContext):
    ...
    bias = read_values_from_file(f"layer{1}-conv{1}bn{1}-bias", cryptoContext.L - res1.cur_limbs, 1, res1.slots,
                                 cryptoContext, scale)
    ...
    bias = read_values_from_file(f"layer{1}-conv{2}bn{2}-bias", cryptoContext.L - res1.cur_limbs, 1, res1.slots,
                                 cryptoContext, scale)
    res1 = fhe.homo_add_pt(res1, bias, cryptoContext)
    res1 = fhe.homo_add(
    ...
    bias = read_values_from_file(f"layer{2}-conv{1}bn{1}-bias", cryptoContext.L - res2.cur_limbs, 1, res2.slots,
                                 cryptoContext, scale)
    ...
    bias = read_values_from_file(f"layer{2}-conv{2}bn{2}-bias", cryptoContext.L - res2.cur_limbs, 1, res2.slots,
                                 cryptoContext, scale)
    res2 = fhe.homo_add_pt(res2, bias, cryptoContext)
    res2 = fhe.homo_add(
    ...
    bias = read_values_from_file(f"layer{3}-conv{1}bn{1}-bias", cryptoContext.L - res3.cur_limbs, 1, res3.slots,
                                 cryptoContext, scale)
    ...
    bias = read_values_from_file(f"layer{3}-conv{2}bn{2}-bias", cryptoContext.L - res3.cur_limbs, 1, res3.slots,
                                 cryptoContext, scale)
    res3 = fhe.homo_add_pt(res3, bias, cryptoContext)
    res3 = fhe.homo_add(
...
def layer2(input, cryptoContext):
    ...
    bias = read_values_from_file(f"layer{4}-conv{1}bn{1}-bias{1}", cryptoContext.L - res1sx0.cur_limbs, 1,
                                 res1sx0.slots, cryptoContext, scaleSx)
    res1sx0 = fhe.homo_add_pt(res1sx0, bias, cryptoContext)
    res1sx1 = conv(boot_in, 32, 1, 16, -1024, 4, 1, 16, scaleSx, cryptoContext)
    bias = read_values_from_file(f"layer{4}-conv{1}bn{1}-bias{2}", cryptoContext.L - res1sx1.cur_limbs, 1,
                                 res1sx1.slots, cryptoContext, scaleSx)
    ...
    bias = read_values_from_file(f"layer{4}-conv{2}bn{2}-bias", cryptoContext.L - fullpackSx.cur_limbs, 1,
    ...
    bias = read_values_from_file(f"layer{5}-conv{1}bn{1}-bias", cryptoContext.L - res2.cur_limbs, 1, res1.slots,
                                 cryptoContext, scale)
    res2 = fhe.homo_add_pt(res2, bias, cryptoContext)
    ...
    bias = read_values_from_file(f"layer{5}-conv{2}bn{2}-bias", cryptoContext.L - res2.cur_limbs, 1, res2.slots,
                                 cryptoContext, scale)
    res2 = fhe.homo_add_pt(res2, bias, cryptoContext)
    res2 = fhe.homo_add(
        res2, fhe.homo_mul_scalar_double(res1, scale, cryptoContext), cryptoContext
    ...
    bias = read_values_from_file(f"layer{6}-conv{1}bn{1}-bias", cryptoContext.L - res3.cur_limbs, 1, res2.slots,
                                 cryptoContext, scale)
    res3 = fhe.homo_add_pt(res3, bias, cryptoContext)
    ...
    bias = read_values_from_file(f"layer{6}-conv{2}bn{2}-bias", cryptoContext.L - res3.cur_limbs, 1, res3.slots,
                                 cryptoContext, scale)
    res3 = fhe.homo_add_pt(res3, bias, cryptoContext)
    res3 = fhe.homo_add(
        res3, fhe.homo_mul_scalar_double(res2, scale, cryptoContext), cryptoContext
...
def layer3(input, cryptoContext):
    ...
    bias = read_values_from_file(f"layer{7}-conv{1}bn{1}-bias{1}", cryptoContext.L - res1sx0.cur_limbs, 1,
                                 res1sx0.slots, cryptoContext, scaleSx)
    res1sx0 = fhe.homo_add_pt(res1sx0, bias, cryptoContext)
    res1sx1 = conv(boot_in, 16, 1, 32, -256, 7, 1, 32, scaleSx, cryptoContext)
    bias = read_values_from_file(f"layer{7}-conv{1}bn{1}-bias{2}", cryptoContext.L - res1sx1.cur_limbs, 1,
                                 res1sx1.slots, cryptoContext, scaleSx)
    res1sx1 = fhe.homo_add_pt(res1sx1, bias, cryptoContext)
    ...
    bias = read_values_from_file(f"layer{7}-conv{2}bn{2}-bias", cryptoContext.L - fullpackSx.cur_limbs, 1,
                                 fullpackSx.slots, cryptoContext, scaleDx)
    fullpackSx = fhe.homo_add_pt(fullpackSx, bias, cryptoContext)
    res1 = fhe.homo_add(fullpackSx, fullpackDx, cryptoContext)
    res1 = fhe.homo_bootstrap(
    ...
    bias = read_values_from_file(f"layer{8}-conv{1}bn{1}-bias", cryptoContext.L - res2.cur_limbs, 1, res2.slots,
                                 cryptoContext, scale)
    res2 = fhe.homo_add_pt(res2, bias, cryptoContext)
    res2 = fhe.homo_bootstrap(
    ...
    bias = read_values_from_file(f"layer{8}-conv{2}bn{2}-bias", cryptoContext.L - res2.cur_limbs, 1, res2.slots,
                                 cryptoContext, scale)
    res2 = fhe.homo_add_pt(res2, bias, cryptoContext)
    res2 = fhe.homo_add(
        res2, fhe.homo_mul_scalar_double(res1, scale, cryptoContext), cryptoContext
    ...
    bias = read_values_from_file(f"layer{9}-conv{1}bn{1}-bias", cryptoContext.L - res3.cur_limbs, 1, res3.slots,
                                 cryptoContext, scale)
    res3 = fhe.homo_add_pt(res3, bias, cryptoContext)
    res3 = fhe.homo_bootstrap(
    ...
    bias = read_values_from_file(f"layer{9}-conv{2}bn{2}-bias", cryptoContext.L - res3.cur_limbs, 1, res3.slots,
                                 cryptoContext, scale)
    res3 = fhe.homo_add_pt(res3, bias, cryptoContext)
    res3 = fhe.homo_add(
        res3, fhe.homo_mul_scalar_double(res2, scale, cryptoContext), cryptoContext
...
def final_layer(input, cryptoContext):
    weight = read_fc_weight(64, 64, cryptoContext.L - input.cur_limbs, 1, input.slots, cryptoContext)
